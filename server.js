...
function fileTransferLoop(tunnel, res, callback){
  ...
  if(!tunnel['complete'])
	  tunnel.uploader.emit("transferUpMore");
  else{
...
app.get("/tunnel/:tunnelId", function(req, res){
    ...
    var headers = {'Content-Type': tunnel['file']['type'], "Content-Disposition": "attachment;filename*=\"" + tunnel['file']['name'] + "\""};
    var userAgent = (req.headers['user-agent']||'').toLowerCase();
    if(userAgent.indexOf('msie') >= 0 || userAgent.indexOf('chrome') >= 0) {
        headers['Content-Disposition'] = 'attachment; filename=' + encodeURIComponent(tunnel['file']['name']);
    } else if(userAgent.indexOf('firefox') >= 0) {
        headers['Content-Disposition'] = 'attachment; filename*="utf8\'\'' + encodeURIComponent(tunnel['file']['name'])+'"';
    } else {
        /* safari and other browsers */
        headers['Content-Disposition'] = 'attachment; filename=' + new Buffer(tunnel['file']['name']).toString('binary');
    }
    ...
    res.on("close", function(){
      ...
      if(successful)
			  tunnels[tunnelId].uploader.emit("tunnelClosed");
      else
        tunnels[tunnelId].uploader.emit("tunnelAborted");
...
io.on('connection', function(socket){
  ...
  socket.on('disconnect', function(){
    if(socketStates[socket.id] != undefined){
      ...
      if(socketStates[socket.id].partner != undefined){
       socketStates[socket.id].partner.emit("refresh", "Your partner decided to disconnect"); 
      }
  ...
  socket.on('stateReply', function(s){
	...
	socket.on('connectRequest', function(targetId){
		...
		users[targetId].emit("incomingConnect", {"partnerId": socketStates[socket.id].selfId});
	...
	socket.on('transferAccept', function(targetId){
		...
		var tunnelId = createTunnel(socketStates[socket.id].partner, socket, socketStates[socketStates[socket.id].partner.id].pendingFile);
